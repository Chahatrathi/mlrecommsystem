...
def get_combined_boundaries(df, top_models_list):
    ...
    # Coarser grid for Vercel speed (h=0.2)
    h = .2  
...
def download_kaggle_dataset(url):
    ...
    if 'datasets' in parts:
        ...
        for file in os.listdir(download_path):
            if file.endswith('.csv'): return pd.read_csv(os.path.join(download_path, file))
    return None
...
def evaluate_and_recommend(df):
    ...
    # SUPER LIGHTWEIGHT: Max 300 rows for Vercel
    if len(df) > 300:
...
def home():
    ...
    if request.method == "POST":
        ...
        try:
            ...
            if df is not None: results, decision_json = evaluate_and_recommend(df)
            else: error = "Invalid dataset source."
